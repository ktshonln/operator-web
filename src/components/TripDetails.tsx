...
function TripDetails() {
  ...
  const { data: trips, isLoading: tripsLoad } = useTrips({
    ...
    const { data: tickets, isLoading: ticketsLoad } = useTickets({tripId: trip?.tripId} as TicketQuery);
    ...
    <div className="flex  space-x-3">
      <div className=" ml-3 mr-10 mt-5 grow">
        ...
        <div className="text-sm space-y-2">
          ...
          <p className="text-neutral-400 font-semibold flex justify-between">
            Express: <span className="text-black font-normal">{trip?.express?'Enabled':'Not Enabled'}</span>
          </p>
          <p className="text-neutral-400 font-semibold flex justify-between">
            Intermediate stops:{" "}
            <span className="text-black font-normal">{trip?.intermediateStops.length!==0? trip?.intermediateStops.map(stop=>`${stop},`) :'None'}</span>
          </p>
          <p className="text-neutral-400 font-semibold flex justify-between">
            Bus:{" "}
            <span className="text-black font-normal">
          ...
          <p className="text-neutral-400 font-semibold flex justify-between">
            Departure date and time: <span className="text-black font-normal">{format(trip?.departureDateAndTime??0, "d/M/yyyy HH'H'mm")}</span>
          </p>
          <p className="text-neutral-400 font-semibold flex justify-between">
         ...
         {trip?.autoScheduling &&
            <>
             <p className="text-neutral-400 font-semibold flex justify-between">
            Departure time: <span className="text-black font-normal">{isValid(new Date(trip?.departureTime??0)) ? format(new Date(trip?.departureTime??0), "HH'H'mm"): 'Invalid date'}</span>
          </p>
          <p className="text-neutral-400 font-semibold flex justify-between">
      ...
      <WidgetLayout>
        <div>
          {
            trip?.autoScheduling && <div className="mb-10">
          ...
          <div className="w-full mt-7 overflow-x-hidden">
            <table className="text-sm w-full">
              ...
              <tbody>
                {trips?.map(({ route, departureDateAndTime, tripId }, i) => (
                  <tr
                    key={tripId + i}
                    onClick={() => {
          ...
          <div className="w-full mt-7 overflow-x-hidden">
            <table className="text-sm w-full">
              ...
              <tbody>
                {tickets?.tickets?.map(({ ticketId, passenger, departureTime}, i) => (
                  <tr
                    key={ticketId??'' + i}
                    onClick={() => {
          ...
          {ticketsLoad && <p>Tickets Loading...</p>}
            ...
            : <p className="text-sm text-center text-neutral-500">No tickets sold yet for this trip</p>
          }
        </div>
      </WidgetLayout>
      {editTrip && trip && (
