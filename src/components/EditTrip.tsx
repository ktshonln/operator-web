...
const schema = z.object({
  route: z.object({
    start: z
      .string()
      .min(2, { message: "Please enter a valid origin." }),
    end: z
      .string()
  ...
  plateNumber: z
    ...
    departureTime: z
      ...
      minuteInterval: z
      .number()
      .min(1, { message: "Please enter a valid minute interval." }).optional(),
      timeRange: z.object({
        from: z
        .string()
  ...
  if(!data.scheduleBlock || !['day', 'week', 'month'].includes(data.scheduleBlock)){
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
    ...
    >
     <>
          ...
          <div className="mb-5">
            ...
            {user.role === "admin" ? (
               ...
               />
                          ...
                          <input className="text-neutral-500" disabled value={camelCaseToTitle(user.branch??"")}/>
                        )}
            </div>
            {errors.route?.start && (
                  <p className="text-red-500 text-xs">{errors.route?.start.message}</p>
                )}
          </div>
          ...
          <div className="relative mb-5">
                  ...
                  <p className="text-red-500 text-xs">{errors.departureDateAndTime.message}</p>
                )}
                                       </div>
                                       <div className="mb-5">
                  ...
                  <p className="text-red-500 text-xs">{errors.autoScheduling.message}</p>
                )}
                                       </div>
                 {autoschedule &&  <>
          ...
          Departure time  <span className="text-red-500 text-base">*</span>
                  ...
                  <div className="relative mb-5">
                                         ...
                                         >
                                           <p>
                                             {valTime ? format(valTime as Date, "pp") : "Choose time"}
                                           </p>
                                           <BiTime size={14} />
                                         </div>
                                         <div className="absolute right-0 z-20 top-5">
                  ...
                  <p className="text-red-500 text-xs">{errors.departureTime.message}</p>
                                       ...
                                       <input  {...register("scheduleBlock")} type="radio"  id="day" value='day' /><label htmlFor="day" className="flex mt-0.5">Day ahead</label>
                                       </div>
                                       <div className=" flex items-center space-x-0.5">

                                       <input {...register("scheduleBlock")} type="radio" id="week" value='week'/><label htmlFor="week" className="flex mt-0.5">Week ahead</label>
                                       </div>
                                       <div className=" flex items-center space-x-0.5">

                                       <input {...register("scheduleBlock")} type="radio" id="month" value='month'/><label htmlFor="month" className="flex mt-0.5">Month ahead</label>
                                       </div>
                                       </div>
                                       {errors.scheduleBlock && (<p className="text-red-500 text-xs">{errors.scheduleBlock.message}</p>)}
                                       </div>
                                       <label htmlFor="dayRange.from" className="block mb-0.5 font-medium">
                                       Day range <span className="text-neutral-400 font-normal">{`(default: everyday)`}</span>
          </label>
          <div className="mb-5">
                                       ...
                                       {errors.dayRange?.from && (<p className="text-red-500 text-xs">{errors.dayRange.from.message}</p>)}
                                       {errors.dayRange?.to && (<p className="text-red-500 text-xs">{errors.dayRange.to.message}</p>)}
          </div>
          ...
          <input {...register("minuteInterval", {valueAsNumber: true})} onChange={(e)=>setminuteVal(parseInt(e.target.value))} type="number" min={1} max={60} className="ring ring-gray-200 mb-5 p-1 rounded-xs bg-white w-12 outline-none"/>
            <label htmlFor="minuteInterval">{(minuteVal===1) ?'Minute' :'Minutes'}</label>
          </div>
          {errors.minuteInterval && (<p className="text-red-500 text-xs">{errors.minuteInterval.message}</p>)}
          </div>
          <label htmlFor="timeRange.from" className="block mb-0.5 font-medium">
                                       Time range <span className="text-neutral-400 font-normal">{`(default: entire day)`}</span>
          </label>
          <div className="mb-5">
          <div className="flex items-center justify-between">
                                       ...
                                       {errors.timeRange?.from && (<p className="text-red-500 text-xs">{errors.timeRange.from.message}</p>)}
                                       {errors.timeRange?.to && (<p className="text-red-500 text-xs">{errors.timeRange.to.message}</p>)}
          </div>
          </>}
                  </>
                  }
