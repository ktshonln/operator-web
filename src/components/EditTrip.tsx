...
const schema = z
  .object({
    ...
    scheduleBlock: z
      .enum(scheduleBlocks, { message: "Please select a schedule block." })
      .optional(),
    dayRange: z
      .object({
        from: z
...
const EditTrip = ({ effectTwo , trip }: Props) => {
  ...
  const { data: routes } = useRoutes(resolvedCompanyId, {} as RouteQuery);

  const [matchingEnds, setMatchingEnds] = useState<string[]>(
    trip.route.start ? [`${trip.route.start}`] : [],
  );
  const [autoschedule, setAutoSchedule] = useState(
    trip?.autoScheduling ? true : false,
  );
  console.log("AutoSIKEDI", autoschedule);

  const [open, setOpen] = useState(false);
  const [val, setVal] = useState<Date | [Date, Date] | null>(
    new Date(trip.departureDateAndTime),
  );

  const [openTime, setOpenTime] = useState(false);
  const departureTimeValue = trip.departureTime
    ? new Date(trip?.departureTime ?? "")
    : null;
  const [valTime, setValTime] = useState<Date | [Date, Date] | null>(
    departureTimeValue,
  );

  const [minuteInterval, setminuteInterval] = useState(
    trip.minuteInterval ? true : false,
  );
          ...
          <>
            ...
            <div className="mb-5">
              <div className="flex items-center justify-between">
                <select
                  ...
                  defaultValue={trip?.dayRange?.from}
                  id="dayRange.from"
            ...
            <div className="flex items-center space-x-2 mb-5">
              <input
                ...
                name="minuteInterval"
                defaultChecked={trip.minuteInterval ? true : false}
                id="minuteInterval"
                className="size-5"
              />
              <span className="flex items-center">
                ...
                <div className="mb-5">
                  <div className="flex items-center justify-between">
                    <select
                      ...
                      defaultValue={trip?.timeRange?.from}
                      id="timeRange.from"
                      className="border-1 border-neutral-200 rounded-sm justify-between p-1 text-sm flex items-center space-x-2 cursor-pointer outline-none"
                    >
                      <option value="">From</option>
                    ...
                    <select
                      ...
                      defaultValue={trip?.timeRange?.to}
                      id="timeRange.to"
                      className="border-1 border-neutral-200 rounded-sm justify-between p-1 text-sm flex items-center space-x-2 cursor-pointer outline-none"
                    >
                      <option value="">To</option>
