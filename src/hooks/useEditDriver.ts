...
const useEditDriver = (companyId: string, driverId: string) => {
    const queryClient = useQueryClient()
  const showToast = useToastStore((state) => state.showToast);
  return useMutation<Driver, Error, DriverDetails, EditDriverContext>({
    mutationFn: (driverDetails: DriverDetails) =>
      apiClient.editBus<DriverDetails>(driverDetails, companyId, driverId),
    onMutate:(newData)=>{ // Optimistic updates
        const previousDrivers = queryClient.getQueryData<Driver[]>(CACHE_KEY_DRIVERS)||[] 
